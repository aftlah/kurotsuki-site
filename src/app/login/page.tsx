"use client";

import Link from "next/link";
import { useEffect, useState } from "react";
import { signIn } from "next-auth/react";
import { Button } from "@/components/Button";
import { Input } from "@/components/Input";
import { AuthLayout } from "@/components/AuthLayout";
import { KatanaDivider } from "@/components/KatanaDivider";
import { useToast } from "@/components/Toast";

export default function LoginPage() {
  const { success, error: toastError, info } = useToast();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const [discordLoading, setDiscordLoading] = useState(false);

  const discordLoginEnabled =
    process.env.NEXT_PUBLIC_DISCORD_LOGIN === "true";

  useEffect(() => {
    const authError = new URLSearchParams(window.location.search).get("error");
    if (authError === "AccessDenied") {
      toastError("Login Discord ditolak. Pastikan email Discord terverifikasi.");
    } else if (authError) {
      toastError("Login gagal. Coba lagi.");
    }
  }, [toastError]);

  const handleDiscordLogin = async () => {
    if (!discordLoginEnabled) {
      info("Login Discord belum dikonfigurasi.");
      return;
    }

    setDiscordLoading(true);
    try {
      await signIn("discord", { callbackUrl: "/dashboard" });
    } catch {
      toastError("Login Discord gagal. Coba lagi.");
      setDiscordLoading(false);
    }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    setLoading(true);

    try {
      const result = await signIn("credentials", {
        email: email.trim().toLowerCase(),
        password,
        redirect: false,
      });

      if (result?.error) {
        const msg = "Email atau kata sandi tidak valid";
        setError(msg);
        toastError(msg);
      } else {
        success("Login berhasil. Mengalihkan...");
        window.location.href = "/dashboard";
      }
    } catch {
      const msg = "Terjadi kesalahan. Silakan coba lagi.";
      setError(msg);
      toastError(msg);
    } finally {
      setLoading(false);
    }
  };

  return (
    <AuthLayout
      title="Welcome Back"
      subtitle="Masuk ke akun Anda"
      footer={
        <p className="text-center text-sm text-gray-muted">
          Belum punya akun?{" "}
          <Link
            href="/register"
            className="font-medium text-crimson transition-colors hover:text-dragon-red"
          >
            Daftar
          </Link>
        </p>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        {error && (
          <div className="rounded-xl border border-danger bg-danger/20 p-3 text-center text-sm text-white-soft">
            {error}
          </div>
        )}

        <Input
          label="Email"
          type="email"
          value={email}
          onChange={(e) => setEmail(e.target.value)}
          placeholder="you@example.com"
          required
          disabled={loading}
        />

        <Input
          label="Kata Sandi"
          type="password"
          value={password}
          onChange={(e) => setPassword(e.target.value)}
          placeholder="••••••••"
          required
          disabled={loading}
          minLength={8}
        />

        <div className="flex items-center justify-between">
          <label className="flex items-center gap-2 text-sm text-gray-muted">
            <input
              type="checkbox"
              className="rounded border-border text-crimson focus:ring-crimson"
              disabled={loading}
            />
            Ingat saya
          </label>
          <Link
            href="/forgot-password"
            className="text-sm text-crimson transition-colors hover:text-dragon-red"
          >
            Lupa kata sandi?
          </Link>
        </div>

        <Button type="submit" className="w-full" size="lg" disabled={loading}>
          {loading ? "Memproses..." : "Masuk"}
        </Button>
      </form>

      <KatanaDivider className="my-8" />

      <Button
        type="button"
        variant="outline"
        className="w-full"
        size="lg"
        disabled={loading || discordLoading || !discordLoginEnabled}
        onClick={handleDiscordLogin}
      >
        <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
          <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
        </svg>
        {discordLoading ? "Menghubungkan..." : "Masuk dengan Discord"}
      </Button>
      {!discordLoginEnabled && (
        <p className="mt-2 text-center text-xs text-gray-muted">
          Discord OAuth belum dikonfigurasi di server.
        </p>
      )}
    </AuthLayout>
  );
}
